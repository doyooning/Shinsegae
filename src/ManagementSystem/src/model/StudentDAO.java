...
public class StudentDAO implements Student {
    ...
    public void update(PersonVO personVO) {
        ...
        try {
            ...
            // UPDATE SQL 직접 작성
            String sql = "update student set sno = ?, name = ?, korean = ?, english = ?, math = ?, science = ? where sno = ?";
            pstmt = conn.prepareStatement(sql);
            ...
            pstmt.setInt(5, newStudent.getMath());
            pstmt.setInt(6, newStudent.getScience());
            pstmt.setString(7, newStudent.getSno());
        ...
        try {
            ...
            if (result == 0) {
                ...
                for (StudentVO student : studentlist) {
                    if (student.getSno().equals(deleteNum)) {
                        // 리스트에서 제거
    ...
    public void search(String searchNum) {
        ...
        for (StudentVO student : studentlist) {
            if (student.getSno().equals(searchNum)) {
    ...
    public void sort(int sortNum) {
        ...
        if (sortNum == 1) {
            studentlist.stream().sorted(Comparator.comparing(StudentVO::getName)).forEach(student -> {
                System.out.println(student);
            });
        } else if (sortNum == 2) {
            studentlist.stream().sorted(StudentVO::compareTo).forEach(student -> {
                System.out.println(student);
            });
        } else if (sortNum == 3) {
            studentlist.stream().sorted(Comparator.comparingInt(StudentVO::getTotal).reversed()).forEach(student -> {
                System.out.println(student);
            });
        } else {
            System.out.println("sort num is incorrect");
        }
    }
